 ...
 */
function generateHtml ({links=['wechat', 'weibo', 'linkedin', 'facebook', 'twitter'], sprite='/bower_components/ftc-social-images/dist/social-images-sprite.svg', destDir='views/partials'} = {})  {
...
// An arrow function. It accepts a `social` object, returns a constructed string.
  ...
  <li class="o-share__action o-share__${social.name}">
    <a href="${social.url}" target="_blank" title="分享到${social.text}">    
      ${sprite ? `<svg><use xlink:href="${sprite}#${social.name}"></use></svg>` : ''}
      <i>${social.text}</i>
    </a>
  </li>
 `;
